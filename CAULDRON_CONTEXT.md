...
Old lab (`cauldron_balance_lab.html`) kept for history; v2 supersedes it.
...
Footprint at game start hit 2859MB again. Audit found the §70/§70b machinery fully intact (budgeted caches, quantized keys, watchdog, safe-by-default raw loader) — the relapse came from the REMAINING full-res render paths the July-4 pass never converted, all of them on screen from launch, silently absorbing every NEW big-canvas PNG drawn since: scene background (`bgtest1`/`customerbg`), the ENTIRE fire meter's flame frames (SwiftUI `Image("name")` = full-res system-cache decode per drawn frame — up to 5 flames × lit+out sets), the composure bar art (bg/fill×4/shield), time-of-day icon + settings button, HP/attack badges + `badgeImage` helper, the potion-bottle banner set (outline/damage/atk/hp_damage) + `banner_border`, the debug button, and the 3D dice face textures (SceneKit pins its material images for the scene's lifetime). ALL converted to `loadDisplayImage(named:displaySize:)` with real display sizes (face textures capped ~1024px). One `UIImage(named:)` remains in CustomerSceneView (~line 39): a run-once asset-name diagnostic print — intentionally left. **Rule clarified: SwiftUI `Image("string")` counts as an unbounded load for drawn art — never use it for anything that might be authored on a big canvas.** Files: PotionShopFireMeterView, PotionShopCustomerSceneView, PotionShopHeaderView, PotionShopGameView, PotionShopCauldronView. HONEST FLAG: could not profile in-hand — if the gauge still reads high after this, report (a) whether "Purge image caches now" moves the number, (b) whether the red watchdog banner ever appeared, (c) what screen was up; that triage separates cache-resident memory from system/SceneKit-pinned memory.

### 70d MEMORY v5 — decode-at-size; the Xcode-attached-run mystery SOLVED (July 5, 2026)
User observation that cracked it: the huge footprint (2819–2859MB) appears ONLY on the first run installed/launched FROM XCODE; a home-screen relaunch is always healthy. Mechanism: the downsampler's redraw step (`base.draw(in:)`) forces a FULL-RESOLUTION decode of every asset into APPLE's named-image cache before shrinking it. iOS reclaims those decodes under normal memory pressure (hence healthy relaunches) — but while a DEBUGGER IS ATTACHED, pressure-driven reclaim is suspended, so every asset's full decode accumulates and neither the watchdog nor "Purge image caches now" can help (it's Apple's cache, not ours). Fix (PotionShopModels, `downsampledImage`): the redraw is replaced with **`UIImage.preparingThumbnail(of:)`** — ImageIO decodes DIRECTLY at target size; the full-res bitmap is never materialized on any run. Renderer redraw kept only as a fallback for exotic sources. §70b's watchdog/purge/budgets unchanged. **Diagnostic rule for the future: a big memory number on an Xcode-attached run that vanishes on home-screen relaunch = decodes pending reclaim, NOT a leak — but decode-at-size should keep even attached runs low now.** Expectation after this: first-from-Xcode footprint in the low hundreds of MB. If it's still ~GB+, the remaining pin is outside image decoding (SceneKit materials on 3D rounds would be next to inspect).

### 58b EDNAR BREATH FADE FIX (July 5, 2026)
Ednar was FADING in and out on a steady ~2.6s rhythm. Cause: the idle breath (§7 of BrewAnimator) was started via `withAnimation(.repeatForever)` inside `.onAppear` — the known SwiftUI trap where the forever-repeat leaks beyond the intended property onto the view's rendering (manifesting as an opacity pulse instead of the intended 1.5% scale swell). Fix (PotionShopCustomerSceneView, PotionShopEdnarView): onAppear now does a PLAIN `ednarBreathPhase = ednarBreathScale` assignment, and a scoped `.animation(.easeInOut(duration: ednarBreathDuration).repeatForever(autoreverses: true), value: ednarBreathPhase)` sits DIRECTLY on the art's scaleEffect — the repeat is pinned to that one subtree and physically cannot touch opacity. The reaction pop (bounded withAnimation) is unchanged and coexists fine (the scoped animation only fires on breathPhase changes). **RULE: never start a repeatForever via withAnimation-in-onAppear; always use the value-scoped .animation form on the exact modifier being animated.** All breath/pop tuning dials in PotionShopBrewAnimator unchanged.

### 58c EDNAR FADE FIX v2 — pose crossfade + rhythm, both killed (July 5, 2026)
§58b's restructure was NOT enough — the fade persisted AND Ednar cross-dissolved between poses. Two mechanisms, both now fixed in PotionShopEdnarView:
1. **Pose cross-dissolve**: the brew sequence's own withAnimation transactions (which animate the rest of the fight) were sweeping up Ednar's expression-image swap. Fix: `.animation(nil, value: expressionAssetName)` directly on the art — pose swaps are INSTANT regardless of any active transaction.
2. **Rhythmic fade**: attaching repeatForever to the image's modifier chain leaks as an opacity pulse in EITHER form (withAnimation-in-onAppear AND value-scoped .animation). Fix: the breath is now a **`.phaseAnimator([1.0, ednarBreathScale])`** applying scaleEffect inside its closure — scoped by construction, self-starting (no onAppear kick), physically cannot touch opacity. `ednarBreathPhase` @State deleted; `ednarPopPhase` (reaction pop, bounded withAnimation) unchanged. Tuning dials in BrewAnimator §7 unchanged and still live.
**RULES (supersede §58b's rule): (a) never attach ANY repeatForever to a chain containing an Image — use phaseAnimator with the effect inside the closure; (b) any image whose CONTENT swaps at runtime (pose/expression/state art) needs `.animation(nil, value: <the swap key>)` or ambient transactions will crossfade it.**

### 58d EDNAR FADE FIX v3 — the crossfade actually killed (July 5, 2026)
v2's `.animation(nil, value:)` did NOT stop the pose crossfade — SwiftUI treats an Image's CONTENT swap as its own rendering category and the brew-sequence transactions still dissolved it. v3 stacks three guards on the art (PotionShopEdnarView): (1) `.contentTransition(.identity)` — content swaps render instantly; (2) the v2 `.animation(nil, value: expressionAssetName)` kept as belt-and-suspenders; (3) **the hard guard: `.id(expressionAssetName)` + `.transition(.identity)`** — each pose is a structurally DIFFERENT view, torn down and re-inserted with an explicit no-effect transition, leaving SwiftUI nothing to interpolate regardless of active transactions. The emoji fallback branch got `.id` + `.transition(.identity)` too (a missing pose asset flips art↔emoji — that swap must be instant as well). Side effect of the id-swap: the breath phaseAnimator restarts at each pose change (a 1.5% swell resetting — imperceptible). **RULE (final form, supersedes §58c-b): runtime-swapped art needs the full trio — contentTransition(.identity) + id(swapKey) + transition(.identity). animation(nil) alone is insufficient for Image content.**

### 71b DAY-END / WIN / LOSE FLAVOR MESSAGES (July 5, 2026)
The day-complete, run-won, and defeat overlays now open with a flavor line. Day-won + run-won lines are picked by **end-of-day composure fraction** (30/30 → "Not a scratch. That was easy!" … 2/30 → "Exhausting day. Tea, then bed."); defeat lines (composure is always 0) vary by **how far the run got** (byDay thresholds 3/7/14/21/30). All lines live in **`PotionShopDayEndMessages`** at the bottom of PotionShopData.swift — pure user-editable dialogue, rows are (threshold, text), any count works, lookups need no touching. PITFALL HANDLED: both the between-round rest heal and the Mended Spirit relic restore composure BEFORE `phase = .dayWon`, so `gs.composure` at overlay time lies; a new **`gs.dayEndComposure`** snapshot is captured at the TOP of `advanceRound` (before any restore) and the overlays read that. Not persisted in the save (only matters within the boundary it's shown at). Also confirmed this session: the old original cast (mildred/bakasura/… incl. carmilla/grimdrek/royal_envoy) is DORMANT — defined in `characters` but nothing spawns them; the campaign draws exclusively from the gmarker `campaignCast`, with boss STAND-INS (bull/skull/fox, finale fishguy) until real bosses are drawn + bucketed. Files: PotionShopGameState, PotionShopGameView, PotionShopData.

### 71c CAST FINALIZED + RELIC/BOON AUDIT (July 5, 2026)
**CAST (user decision, July 5): the gmarker characters ARE the final cast.** The old pre-template originals — mildred, bakasura, and the rest, INCLUDING carmilla/grimdrek/royal_envoy — were redrawn as gmarker characters and are permanently retired (definitions remain in `characters` as dormant data; nothing spawns them). The boss "stand-ins" (bull/skull/fox weekly, fishguy finale) are the real boss closers; Data comments updated (property names kept to avoid call-site churn).
**BOON/RELIC AUDIT — every card checked against its blurb.** Verified working: Mended Spirit (day-end restore fires before the day-won screen; §71b's dayEndComposure keeps the flavor message honest), Warded Morning (advanceDay `shield += 5`, nothing wipes it — note it ADDS to leftover shield), typeWideBonus + per-die rule bonuses (consumed at every brew site: base value, boosts, mirror), customFaces riding onto dealt dice. Three FAILURES found + fixed:
1. **Die Upgrade** upgraded only the FIRST die of a type, but seed deck holds 3 potency / 2 stability and the dealer picks randomly within a lane → the upgrade appeared in ~1/3 of potency hands while the picker presented it as type-level. `applyDieUpgrade` now steps EVERY die of the chosen type (exactly one lane die appears per hand, so this makes reality match the picker exactly).
2. **Extra Potency / Extra Heal / Spare Ember were DEAD under type-draw dealing** (die count doesn't change what's dealt; a fresh basic die could dilute an upgraded lane). Replaced with **Potent Brew ⚗️ / Healing Mastery 💚 / Stoked Coals 🔥 = typeWideBonus +1** (half the pre-§67 values; Stoked Coals also fattens fire refills since stability value drives them). ✏️ user-editable rows as always. `.addDie` itself hardened: added dice now inherit the type's best customFaces, so user-authored add-a-die cards can never be traps. BALANCE FLAG: flat +1s are stronger than face steps; §71's lab found player growth is the missing lever, so likely fine — verify on the next lab pass.
3. **Owned one-shot relics could be re-offered** (dead pick). `PotionShopBoonPool.draw(_:owned:)` now filters an owned Mended Spirit; Warded Morning stays offerable (stacks). GameState's offerBoons passes the run.
Files: PotionShopData, PotionShopRunSystem, PotionShopGameState.
