...
struct AnimatedHeroPortrait: View {
    ...
    var body: some View {
        ...
        // 🆕 the bare asset name itself (e.g. "gmarker_oldlady")
        let stateStatic = "\(assetPrefix)_\(state.boilSuffix)"
        let idleStatic = "\(assetPrefix)_idle"

        let fallback: String
        if UIImage(named: stateStatic) != nil {
            fallback = stateStatic
        } else if UIImage(named: idleStatic) != nil {
            fallback = idleStatic
        } else {
            fallback = assetPrefix   // 🆕 bare image name
        }
